(function () {
    ...
    const bridgeSpec = Object.freeze({
        moduleName: "many2x_parent_form_autosave.bridge",
        guardProperty: "__many2xParentFormAutosaveDefined",
        debugProperty: "__many2xParentFormAutosaveDebug",
        logPrefix: "many2x_parent_form_autosave",
        parentFormSelector: ".o_form_view",
        x2manyFieldSelector: ".o_field_x2many",
        modalSelector: ".modal",
        parentSaveMethodName: "saveButtonClicked",
        maxOwnerDepth: 12,
        debugEventLimit: 40,
    });
    ...
    if (!rootOdoo || typeof rootOdoo.define !== "function") {
        console.warn("[" + bridgeSpec.logPrefix + "] Odoo module loader is not available.");
        return;
    }
    if (rootOdoo[bridgeSpec.guardProperty]) {
        return;
    }
    rootOdoo[bridgeSpec.guardProperty] = true;
    rootOdoo[bridgeSpec.debugProperty] = rootOdoo[bridgeSpec.debugProperty] || { events: [] };

    rootOdoo.define(bridgeSpec.moduleName, [
        ...
        const debugState = rootOdoo[bridgeSpec.debugProperty];
        ...
        function pushDebug(eventName, payload) {
            try {
                ...
                if (debugState.events.length > bridgeSpec.debugEventLimit) {
        ...
        function findParentFormNode(component) {
            ...
            const formNode = rootNode.closest(bridgeSpec.parentFormSelector);
            if (!(formNode instanceof HTMLElement)) {
                return null;
            }
            if (formNode.closest(bridgeSpec.modalSelector)) {
                return null;
            }
            return formNode;
        }
        ...
        function resolveInlineEditorContext(component) {
            ...
            const x2manyNode = rootNode.closest(bridgeSpec.x2manyFieldSelector);
            ...
            for (let depth = 0; owlNode && depth < bridgeSpec.maxOwnerDepth; depth += 1) {
                ...
                if (
                    !formController &&
                    owner &&
                    typeof owner[bridgeSpec.parentSaveMethodName] === "function"
                ) {
        ...
        async function persistParentForm(context) {
            ...
            if (!formController || typeof formController[bridgeSpec.parentSaveMethodName] !== "function") {
                pushDebug("persist_parent_missing_controller_method", {
                    method: bridgeSpec.parentSaveMethodName,
                });
                return false;
            }
            try {
                const saved = await formController[bridgeSpec.parentSaveMethodName]({});
                pushDebug("persist_parent_controller_method", { saved: saved !== false });
                return saved !== false;
            } catch (_controllerSaveErr) {
                pushDebug("persist_parent_controller_method_error", {
